locals {
  ...
  schedule_enabled = try(trimspace(var.schedule_expression), "") != ""
  ...
  lambda_filename = local.is_zip && local.use_archive ? try(data.archive_file.lambda_zip[0].output_path, null) : (
    local.is_zip && local.use_local_zip ? var.local_existing_package_path : null
  )

  lambda_source_code_hash = local.is_zip && local.use_archive ? try(data.archive_file.lambda_zip[0].output_base64sha256, null) : (
    local.is_zip && local.use_local_zip ? filebase64sha256(var.local_existing_package_path) : null
  )
}
